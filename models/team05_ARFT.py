...
class ResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = FFT_spital_module(dim)
...
class ARFT_model(nn.Module):
    ...
    def __init__(self,
                ...
                #  upsampler='pixelshuffle',
                 **kwargs):
        super(ARFT_model, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.upsampler ='pixelshuffle' #upsampler
        ...
        if resi_connection == '1conv':
            self.conv_after_body = FFT_spital_module(embed_dim)
    ...
    def forward(self,input):
        ...
        b, c, h, w = input.size()
        if h <= 160 or w <=160:
            raise ValueError('Only support full image test, do not support patch image')
        else:
            ...
            if h - (h_space[-1] + crop_sz) > theat:
                h_space = np.append(h_space, h - crop_sz)
            w_space = np.arange(0, w - crop_sz + 1, step)
            if w - (w_space[-1] + crop_sz) > theat:
                w_space = np.append(w_space, w - crop_sz)

            E = torch.zeros(b, c, h*sf, w*sf).type_as(input)
            W = torch.zeros_like(E)

            for x in h_space:
                for y in w_space:
                    in_patch = input[:,:, x:x + crop_sz, y:y + crop_sz]#[..., h_idx:h_idx+crop_sz, w_idx:w_idx+tile]
                    #print(in_patch.shape)
                    out_patch = self.run(in_patch)
                    out_patch_mask = torch.ones_like(out_patch)

                    E[..., x*sf:(x+crop_sz)*sf, y*sf:(y+crop_sz)*sf].add_(out_patch)
                    W[..., x*sf:(x+crop_sz)*sf, y*sf:(y+crop_sz)*sf].add_(out_patch_mask)
            output = E.div_(W)
        return output
    ...
    def run(self, x):
        ...
        sr_list = [self.single_run(aug) for aug in lr_list]
    ...
    def flops(self):
        ...
        return flops
class ARFT(nn.Module):
    def __init__(self):
        super(ARFT, self).__init__()
