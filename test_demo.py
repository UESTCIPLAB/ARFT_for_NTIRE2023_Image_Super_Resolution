...
def select_model(args, device):
    ...
    if model_id == 5:
        from models.team05_ARFT import ARFT
        name, data_range = f"{model_id:02}_ARFT_baseline", 255.0
      
        model_path = os.path.join('model_zoo', 'team05_ARFT.pt')
        model = ARFT()
        model.load_state_dict(torch.load(model_path), strict=True)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default="DIV_vaild_dataset", type=str)
    parser.add_argument("--save_dir", default="ARFT_for_NTIRE2023_Image_Super_Resolution", type=str)
